...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : "Failed to sign up. Please try again.";
      setError(errMsg);
      setLoading(false);
    }
  };
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Successful verification and sign-in
      router.push("/dashboard");
      router.refresh();
    } catch (err: unknown) {
  ...
  const handleOAuthSignIn = (provider: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        const errMsg = err instanceof Error ? err.message : `Failed to initialize ${provider} registration.`;
        setError(errMsg);
      }
    });
  };
